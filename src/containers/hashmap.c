...
void *standard_malloc(size_t size, void *udata) {
	return kmalloc(size);
}

void *standard_realloc(void *ptr, size_t size, void *udata) {
	return krealloc(ptr, size);
}

void standard_free(const void *ptr, void *udata) {
	kfree(ptr);
}
...
struct hashmap {
	hashmap_allocator malloc;
	hashmap_reallocator realloc;
	hashmap_releaser free;
	bool oom;
	size_t elsize;
	size_t cap;
	uint64_t seed0;
...
struct hashmap *hashmap_new_with_allocator(hashmap_allocator _malloc, hashmap_reallocator _realloc, hashmap_releaser _free, size_t elsize, size_t cap, uint64_t seed0, uint64_t seed1, hashmap_hash_fn hash,
					   hashmap_compare_fn compare, hashmap_elfree_fn elfree, void *udata) {
	_malloc = _malloc ? _malloc : standard_malloc;
	_realloc = _realloc ? _realloc : standard_realloc;
	_free = _free ? _free : standard_free;
	size_t ncap = 16;
	if (cap < ncap) {
		cap = ncap;
	} else {
...
struct hashmap *hashmap_new(size_t elsize, size_t cap, uint64_t seed0, uint64_t seed1, hashmap_hash_fn hash, hashmap_compare_fn compare, hashmap_elfree_fn elfree, void *udata) {
	return hashmap_new_with_allocator(standard_malloc, standard_realloc, standard_free, elsize, cap, seed0, seed1, hash, compare, elfree, udata);
}
