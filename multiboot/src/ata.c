...
unsigned char ide_print_error(unsigned int drive, unsigned char err) {
   ...
   printf(" IDE %d:", drive);
   if (err == 1) {printf("%s", "- Device Fault\n     "); err = 19;}
   else if (err == 2) {
      unsigned char st = ide_read(ide_devices[drive].channel, ATA_REG_ERROR);
      if (st & ATA_ER_AMNF)   {printf("%s", "- No Address Mark Found\n     ");   err = 7;}
      if (st & ATA_ER_TK0NF)   {printf("%s", "- No Media or Media Error\n     ");   err = 3;}
      if (st & ATA_ER_ABRT)   {printf("%s", "- Command Aborted\n     ");      err = 20;}
      if (st & ATA_ER_MCR)   {printf("%s", "- No Media or Media Error\n     ");   err = 3;}
      if (st & ATA_ER_IDNF)   {printf("%s", "- ID mark not Found\n     ");      err = 21;}
      if (st & ATA_ER_MC)   {printf("%s", "- No Media or Media Error\n     ");   err = 3;}
      if (st & ATA_ER_UNC)   {printf("%s", "- Uncorrectable Data Error\n     ");   err = 22;}
      if (st & ATA_ER_BBK)   {printf("%s", "- Bad Sectors\n     ");       err = 13;}
   } else  if (err == 3)           {printf("%s", "- Reads Nothing\n     "); err = 23;}
     else  if (err == 4)  {printf("%s", "- Write Protected\n     "); err = 8;}
   printf("- [%s %s] %s\n",
...
void ide_read_sectors(unsigned char drive, unsigned char numsects, unsigned int lba, unsigned short es, unsigned int edi)
{
   ...
   // ==================================
   if (drive > 3 || ide_devices[drive].reserved == 0) printf("Drive %d not found\n", drive);      // Drive Not Found!
...
void ide_atapi_eject(unsigned char drive)
{
   ...
   // ==================================
   else if (ide_devices[drive].type == IDE_ATA) printf("Command aborted on drive %d\n", drive);         // Command Aborted.
