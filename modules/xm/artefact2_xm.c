...
void xm_reset_context(xm_context_t* ctx) {
	...
	for (uint16_t i = ctx->module.num_instruments; i; --i, ++inst) {
...
void xm_set_sample_rate([[maybe_unused]] xm_context_t* ctx, [[maybe_unused]] uint16_t rate) {
	#if XM_SAMPLE_RATE == 0
...
/* For debugging */
void xm_print_pattern([[maybe_unused]] xm_context_t* ctx, [[maybe_unused]] uint8_t pat) {
}