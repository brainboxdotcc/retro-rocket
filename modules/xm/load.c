...
int xm_pos_wrapped(xm_context_t *ctx, uint8_t *prev_pi, uint8_t *prev_pat, uint8_t *prev_row, uint32_t *prev_samp) {
	uint8_t pi = 0, pat = 0, row = 0;
	uint32_t samp = 0;
	xm_get_position(ctx, &pi, &pat, &row, &samp);

	int wrapped = 0;
	if (*prev_samp != 0) {
		if (pi < *prev_pi) wrapped = 1;
		else if (pi == *prev_pi && pat < *prev_pat) wrapped = 1;
		else if (pi == *prev_pi && pat == *prev_pat && row < *prev_row) wrapped = 1;
	}

	*prev_pi = pi; *prev_pat = pat; *prev_row = row; *prev_samp = samp;
	return wrapped;
}
...
moddata + offset + 129);
instr->volume_envelope.
num_points = READ_U8(offset + 225);
instr->volume_envelope.
sustain_point = READ_U8(offset + 227);
instr->volume_envelope.
loop_start_point = READ_U8(offset + 228);
instr->volume_envelope.
loop_end_point = READ_U8(offset + 229);
...
moddata + offset + 177);
instr->panning_envelope.
num_points = READ_U8(offset + 226);
instr->panning_envelope.
sustain_point = READ_U8(offset + 230);
instr->panning_envelope.
loop_start_point = READ_U8(offset + 231);
instr->panning_envelope.
loop_end_point = READ_U8(offset + 232);
...
case 0:
instr->
vibrato_type = WAVEFORM_SINE;
break;
case 1:
instr->
vibrato_type = WAVEFORM_SQUARE;
break;
case 2:
instr->
vibrato_type = WAVEFORM_RAMP_DOWN;
break;
case 3:
instr->
vibrato_type = WAVEFORM_RAMP_UP;
break;
default:
assert(0);
}
instr->
vibrato_sweep = READ_U8(offset + 236);
instr->
vibrato_depth = READ_U8(offset + 237);
instr->
vibrato_rate = READ_U8(offset + 238);
#endif

#if HAS_FADEOUT_VOLUME
instr->
volume_fadeout = READ_U16(offset + 239);
#endif
...
if(s->length & (1u << 31)) {
s->length &= ~(1u << 31);
if(_Generic((xm_sample_point_t)0,
int8_t: true,
default: false)) {
NOTICE("16 bit sample will be dithered to 8 bits");
}
xm_load_xm0104_16b_sample_data(s
->length, sample_data,
moddata, moddata_length,
offset);
offset += s->index * 2;
} else {
xm_load_xm0104_8b_sample_data(s
->length, sample_data,
moddata, moddata_length,
offset);
offset += s->
...
s->
index = ctx->module.samples_data_length;
ctx->module.samples_data_length += s->
length;
